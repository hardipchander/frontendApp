...
const SingleTaskView=(props) => {
    ...
    return (
        <div className="single-Task-Box">
          ...
          {(task.employee!==null)? (<p className="employee-text"> The employee is <Link to={`/employees/${task.employee.id}`}><span className="fullName">{fullName}</span></Link></p>): (<p className="employee-text">Unassigned.</p>)}
